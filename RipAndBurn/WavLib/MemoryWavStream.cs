...
namespace RipAndBurn.WavLib {
    /// <summary>
    /// Save RAW PCM data to a stream in WAVE format
    /// </summary>
    public class MemWaveWriter : AudioWriter {
        ...
        public MemWaveWriter(Stream Output, WaveFormat Format, uint AudioDataSize)
          : base(Output, Format) {
            m_AudioDataSize = AudioDataSize;
            WriteWaveHeader();
        }

        public MemWaveWriter(Stream Output, WaveFormat Format)
          : base(Output, Format) {
